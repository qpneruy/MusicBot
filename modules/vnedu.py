...
class Vnedu:
    ...
    def _get_data(cls, phone_number: str, password: str, year: str) -> any:
        ...
        data = requests.get(
            f"https://hocbadientu.vnedu.vn/sllservices/index.php?callback=jQuery11240405892063703583_1700195483757&call=solienlac.checkSll&mahocsinh={cls.student_id}&tinh_id=7&password={password}&namhoc={year}&dot_diem_id=0&_=1700195483760")
        ...
        """Lấy dữ liệu sổ điểm"""
        data = requests.get(
            f"https://hocbadientu.vnedu.vn/sllservices/index.php?callback=jQuery11240405892063703583_1700195483757&call=solienlac.getSodiem&mahocsinh={cls.student_id}&key=d33e425220d1f1184a9fb9a477055fd6&namhoc={year}&tinh_id=7&dot_diem_id=0&_=1700195483761",
            cookies=cookies)
    ...
    def get_bang_diem(cls, phone_number: str, password: str, period: int, year: str) -> any:
        score_data = cls._get_data(phone_number, password, year)
        image = Image.open("D:\\Project\\Python\\qpneruy-Git\\modules\\bangdiem.png")
        if "Code1" in score_data[0]:
            return score_data
         if "Code2" in score_data[0]:
            print("adhaihadas")
            return score_data
        print(score_data)
        ...
        ds_namhoc = cls._get_ds_nam_hoc(student_id=cls.student_id)
        ten_lop = ""
        for items in ds_namhoc:
            if items["nam_hoc"] == year:
...
vn.get_bang_diem(phone_number="0838757028", password="0838757028", period=1, year='2029')
